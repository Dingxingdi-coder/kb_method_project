...
本目录提供 H20 正式实验使用的 Codex Hook 观测模块。它的定位是旁路记录器：记录时间、candidate 快照、candidate 修改尝试、compile/correctness/benchmark 调用、以及 harness 结果中的 p50/speedup 字段；它不修改 `candidate.py`，不修改 harness，也不向 Agent 注入额外上下文。

## 文件

```text
tools/codex_h20_hooks/
  hooks.json             # Codex hooks.json 模板
