...
输入：OpSpec、reference、harness 命令、预算限制、从 `raw_corpus_v0` 普通 RAG 召回的原始资料 chunk。
...
输入：OpSpec、reference、harness 命令、预算限制、从 `KB v0` 普通 RAG 召回的知识单元文本。
...
A1、A2、A3 的上下文 token 上限必须相同。若某组召回结果超过上限，检索器必须截断，而不是放宽预算。若某 Agent 不能报告 token，至少记录 prompt/output character count 和 wall-clock time。